 ...
 */
class TimezoneClient(cpars: Seq[ClientParameter]) extends GoogleClient[TimezoneResponse](cpars) {
  ...
  def ?(location: LocationParam, epoch: TimestampParam)(implicit executionContext: ExecutionContext): Future[Either[Error, TimezoneResponse]]  = {
    ...
    validateParameters( cpars ++ Seq(location, epoch) ) flatMap {
      case Left(error) =>
        Future { Left(error) }
...
object TimezoneClientConst {
  ...
  val req = "https://maps.googleapis.com/maps/api/timezone/json"
